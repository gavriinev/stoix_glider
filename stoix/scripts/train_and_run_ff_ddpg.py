...
def main() -> None:
    ...
    if args.load_network:
        ...
        # The loaded config is kept for reference but not used directly to avoid compatibility issues
        ...
        # We need to get the actor_apply_fn from the network setup
        # This requires creating the environment and network architecture
        from stoix.systems.ddpg.ff_ddpg import learner_setup
        from stoix.utils.total_timestep_checker import check_total_timesteps
        
        # Calculate total timesteps and setup config properly
        n_devices = len(jax.devices())
        cfg.num_devices = n_devices
        cfg = check_total_timesteps(cfg)
        
        _ensure_buffer_capacity(cfg)
        env, _ = environments.make(config=cfg)
        key, actor_net_key, q_net_key = jax.random.split(jax.random.PRNGKey(cfg.arch.seed), num=3)
        _, actor_network, _ = learner_setup(env, (key, actor_net_key, q_net_key), cfg)
        actor_apply_fn = actor_network.apply
        
        # The loaded params need to be in the right format for evaluation
        # They should have the device and batch dimensions
    ...
    if args.save_network and not args.skip_training:
    ...
    print(f"Visualization saved to: optimized_simulation.html")
    print("="*80)
